...
def listado_superheroes_orden_descendente(arbol):
    if(arbol.info is not None):
            if(arbol.der is not None):
                listado_superheroes_orden_descendente(arbol.der)
            if(arbol.datos['esHeroe'] == True):
                print(arbol.info)
            if(arbol.izq is not None):
                listado_superheroes_orden_descendente(arbol.izq)
...
def ejercicio6():
    ...
    print('MOSTRAR LOS VILLANOS ALFABETICAMENTE')
    ...
    arreglar_nombre_superheroe(arbolMCU, 'Doc', 'Doctor Stranger')
    
    print('\nLISTADO DE SUPERHEROES DE MANERA DESCENDENTE')
    listado_superheroes_orden_descendente(arbolMCU)
