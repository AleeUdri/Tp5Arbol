...
def contar_mayor_cant_de_criaturas_derrotadas_por_heroe(arbol):
...
def modificar_datos_criatura(arbol, campo, criatura, dato):
    pos = arbol.busqueda(criatura)
    if(pos is not None):
...
def modificar_nombre_criatura(arbol, criatura, modificarlo_por):
    info, datos = arbol.eliminar_nodo(criatura)
...
def ejercicio_24():
    ...
    lista_dic_heroes = contar_mayor_cant_de_criaturas_derrotadas_por_heroe(arbol_criaturas)
    ...
    modificar_datos_criatura(arbol_criaturas, 'Cant_Derrotado','Aves del Estinfalo', 10)
    modificar_datos_criatura(arbol_criaturas, 'Derrotado','Aves del Estinfalo', 'Heracles')

    print('\nCAMBIAR EL NOMBRE DE LADON POR DRAGON LADON')
    modificar_nombre_criatura(arbol_criaturas, 'Ladon', 'Dragon Ladon')
